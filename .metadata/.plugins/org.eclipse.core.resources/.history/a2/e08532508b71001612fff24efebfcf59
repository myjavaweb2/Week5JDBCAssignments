...
//		playerToAdd = aboutThePlayer();
		...
		try {
			...
			PREP_STMT.setString(1, player.getFirstName());
			PREP_STMT.setString(2, player.getLastName());
			PREP_STMT.setString(3, player.getCurrentLastTeam());
			PREP_STMT.setString(4, player.getPosition());
			PREP_STMT.setInt(5, player.getTotalPoints());
			PREP_STMT.setInt(6, player.getChampionshipsWon());
			PREP_STMT.setInt(7, player.getMvpWon());
	...
	public static void updateToDB(Player player) {

		readFromDB();

//		System.out.println("Please enter the player ID of the player you would like to update.");
...
//		playerToUpdate = aboutPlayerUpdate();
		...
		try {
			...
			PREP_STMT.setInt(6, player.getChampionshipsWon());
			PREP_STMT.setInt(7, player.getMvpWon());
			PREP_STMT.setString(8, player.getPlayerID());
	...
	public static void deleteFromDB(Player player) {
		
		connectToDB();
		
		readFromDB();

//		System.out.println("Please enter the player ID of the player you would to delete.");
//		userInput = sc.nextLine();
		
		try {
			PREP_STMT = CONN.prepareStatement(deleteFromTable);
			
			PREP_STMT.setString(1, player.getPlayerID());
	...
	private static String deleteFromTable = "DELETE FROM `nba`.`famous_players`"
			+ " WHERE player_id = ?";
